...
<script>
import { AbilityToCall, AbilityToCallDown } from '@/api/data';
export default {
  name: 'Application',
  data () {
    return {
  ...
  methods: {
    ...
    // 重置
    down () {
      AbilityToCallDown().then((res) => {
        let blob = new Blob([res.data])
        let downloadElement = document.createElement('a')
        let href = window.URL.createObjectURL(blob)
        downloadElement.href = href
        downloadElement.setAttribute("download", '能力调用方增长率报表.xlsx');
